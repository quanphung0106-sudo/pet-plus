...
const AlertBase = (props: AlertProps & { value?: string }) => {
  ...
  useEffect(() => {
    enqueueSnackbar(<>{children}</>, {
      ...
      className: className,
      action: (
        <IconButton classes={{ root: styles.AlertIconButton }} size='small' onClick={() => closeSnackbar(value)}>
          <Close />
        </IconButton>
      )
    })
...
export const SuccessNotification = (props: AlertProps & CustomAlertProps) => {
  ...
  return (
    <AlertBase {...rest} className={clsx(styles.AlertBase, styles.AlertSuccessRoot, className)} severity='success'>
...
export const InfoNotification = (props: AlertProps & CustomAlertProps) => {
  const { children, label, className, ...rest } = props
  return (
    <AlertBase {...rest} className={clsx(styles.AlertBase, styles.AlertInfoRoot, className)} severity='info'>
      <Stack className={styles.AlertContent}>
        <AlertTitle classes={{ root: styles.AlertContentLabel }}>{label}</AlertTitle>
        <Typography component='div' classes={{ root: styles.TypographyRoot }}>
          {children}
...
export const WarningNotification = (props: AlertProps & CustomAlertProps) => {
  const { children, label, className, ...rest } = props
  return (
    <AlertBase {...rest} className={clsx(styles.AlertBase, styles.AlertWarningRoot, className)} severity='warning'>
      <Stack className={styles.AlertContent}>
        <AlertTitle classes={{ root: styles.AlertContentLabel }}>{label}</AlertTitle>
        <Typography component='div' classes={{ root: styles.TypographyRoot }}>
          {children}
...
export const ErrorNotification = (props: AlertProps & CustomAlertProps) => {
  const { children, label, className, ...rest } = props
  return (
    <AlertBase {...rest} className={clsx(styles.AlertBase, styles.AlertErrorRoot, className)} severity='error'>
      <Stack className={styles.AlertContent}>
        <AlertTitle classes={{ root: styles.AlertContentLabel }}>{label}</AlertTitle>
        <Typography component='div' classes={{ root: styles.TypographyRoot }}>
          {children}
